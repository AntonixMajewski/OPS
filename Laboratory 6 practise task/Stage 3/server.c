...
void create_server_queues(char *pid_str, mqd_t *s_queue, mqd_t *d_queue, mqd_t *m_queue) {
    ...
    // Create queues
    *s_queue = mq_open(s_queue_name, O_RDONLY | O_NONBLOCK, 0644, NULL);
    *d_queue = mq_open(d_queue_name, O_RDONLY | O_NONBLOCK, 0644, NULL);
    *m_queue = mq_open(m_queue_name, O_RDONLY | O_NONBLOCK, 0644, NULL);
...
void handle_message(mqd_t queue, char *queue_name) {
    ...
    mq_send(client_queue, (const char *)&result, sizeof(result), 0);
